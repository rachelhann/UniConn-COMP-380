...
public interface SearchRepository extends JpaRepository<User, Integer> {
       ...
       @Query("SELECT u FROM User u WHERE " +
              ...
              "ORDER BY CASE WHEN LOWER(u.username) LIKE LOWER(CONCAT(:keyword, '%')) THEN 0 ELSE 1 END")
       List<User> searchUsers(@Param("keyword") String keyword);

       @Query("SELECT c FROM Community c WHERE " +
              "LOWER(c.communityName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
              "LOWER(c.description) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
              "ORDER BY CASE WHEN LOWER(c.communityName) LIKE LOWER(CONCAT(:keyword, '%')) THEN 0 ELSE 1 END")
       ...
       @Query("SELECT p FROM Post p WHERE p.isDeleted = false AND (" +
              ...
              "ORDER BY CASE WHEN LOWER(p.title) LIKE LOWER(CONCAT(:keyword, '%')) THEN 0 ELSE 1 END")
