...
async function performSearch(query) {
  ...
  try {
    ...
    let users = [], communities = [], posts = [], tags = [];

    if (activeFilter === 'all' || activeFilter === 'users' || activeFilter === 'communities' || activeFilter === 'posts') {
      const res = await fetch(`/api/search?q=${encodeURIComponent(query)}`, { headers });
      const data = await res.json();
      if (activeFilter === 'all' || activeFilter === 'users') users = data.users || [];
      if (activeFilter === 'all' || activeFilter === 'communities') communities = data.communities || [];
      if (activeFilter === 'all' || activeFilter === 'posts') posts = data.posts || [];
    }

    if (activeFilter === 'all' || activeFilter === 'tags') {
      const res = await fetch(`/api/search/tags?q=${encodeURIComponent(query)}`, { headers });
      tags = await res.json();
    }
